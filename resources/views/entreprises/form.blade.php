<form action="{{ isset($action) && $action ===  'POST' ?  route('admins_regulateurs.store') :  route('admins_regulateurs.update', $admin) }}"
    ...
    <div class="form-row">
        <div class="form-group col-md-6">
            ...
            <input type="tel" value="{{ (old('phoneNumber')) ? old('phoneNumber') : ((isset($admin) ? $admin->phoneNumber : '')) }}" name="phoneNumber" class="form-control @error('phoneNumber') is-invalid @enderror" id="phoneNumber" placeholder="Téléphone" required="required">
            @error('phoneNumber')
            <span class="invalid-feedback" role="alert">
                <strong>{{ $message }}</strong>
            </span>
